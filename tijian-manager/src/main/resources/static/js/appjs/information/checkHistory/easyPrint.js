...
/**
 * auth:papio
 * 该组件依赖jquery方法，所以使用前先引用jquery
 * 使用方法
 *
 * var print = new EasyPrint({url:""});
 * print.start();
 *
 *
 */
...
;(function(undefined) {
    ...
    EasyPrint.prototype = {
        ...
        start:function(){
            var _this = this;
            $("body").append(_this.messageBox(_this.def.message));
            $("#printMessageBox").css("opacity", 0);
            $("#printMessageBox").animate({opacity:1}, 300, function() { _this.addIframeToPage(); });
        },
        /**
         ...
         * @param {Object} pluginOptions - options for this print button
         */
         addIframeToPage:function(){
            var _this = this;
            var url = _this.def.url;

            if(!$('#printPage')[0]){
                $("body").append(_this.iframe(url));
                $('#printPage').bind("load",function() {  _this.printit();  })
            }else{
         ...
         */
         printit:function(){
               // frames["printID"].focus();
               // frames["printID"].print();
              frames[0].focus();
              frames[0].print();
              frames[1].focus();
              frames[1].print();
        ...
        iframe: function(url){
            let div="";
                for(let u of url) {
                    div+=`<iframe id='printPage' name="printPage" src='${u}' style="position:absolute;top:0px; left:0px;width:0px; height:0px;border:0px;overfow:none; z-index:-1"></iframe>`;
                }
    ...
    if (typeof module !== "undefined" && module.exports) {
        module.exports = EasyPrint;
    } else if (typeof define === "function" && define.amd) {
        define(function(){return EasyPrint;});
    } else {
        !('EasyPrint' in _global) && (_global.EasyPrint = EasyPrint);
    }
}());